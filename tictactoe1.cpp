...
void initialise(int *board,string str)
{
	int i;
	for(i=0;i<25;i++)
		board[i]=BORDER;
	for(i=0;i<9;i++)
		{
			if(str[i]=='3')
				...
				board[convertTo25[i]]=NOUGHTS;
			else if(str[i]=='1')
				board[convertTo25[i]]=CROSSES;
...
int rungame()
{
	int gameover= 0;
	int side=CROSSES;
	if(findthreeinarowallboard(&board[0],side^1)==3)
		{
            return (100);
		}
		if(!Hasempty(&board[0]))
	...
	int lastmovemade=0;
		...
		{
            return (102+lastmovemade);
		}
		if(!Hasempty(&board[0]))
		{
			return(120+lastmovemade);
		}
		return (lastmovemade);
}
